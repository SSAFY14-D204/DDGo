...
def recompute_quat_targets(points: dict[str, Any]) -> dict[str, np.ndarray]:
    ...
    lhand_tip = np.asarray(points["left_hand_tip"], dtype=np.float64)
    rhand_tip = np.asarray(points["right_hand_tip"], dtype=np.float64)

    larm_x, larm_y, larm_z = frame_from_segments(le - ls, lhand_tip - le, up_axis)
    rarm_x, rarm_y, rarm_z = frame_from_segments(re - rs, rhand_tip - re, up_axis)
...
def build_effector_targets(mapped_points: dict[str, Any]) -> dict[str, np.ndarray]:
    return {
        "left_wrist": np.asarray(mapped_points["left_hand_tip"], dtype=np.float64).copy(),
        "right_wrist": np.asarray(mapped_points["right_hand_tip"], dtype=np.float64).copy(),
        "left_ankle": np.asarray(mapped_points["left_toe"], dtype=np.float64).copy(),
        "right_ankle": np.asarray(mapped_points["right_toe"], dtype=np.float64).copy(),
...
def refine_pose_with_ik(
    ...
    hand_left_bid = body_id(model, "hand_left")
    hand_right_bid = body_id(model, "hand_right")
    foot_left_bid = body_id(model, "foot_left")
    foot_right_bid = body_id(model, "foot_right")
...
def run_static_analysis(
    ...
    target_projected_px = project_points_with_affine(ik_points, projection_affine, COMPARISON_POINT_NAMES)
    ...
    render_2d_comparison_overlay(
        frame_bgr=frame_bgr,
        mediapipe_px=image_landmarks_px,
    ...
    load_percentage: list[dict[str, Any]] = []
    ...
    anchor_forces: list[dict[str, Any]] = []
    ...
    for limb, body_name in LIMB_TO_BODY.items():
        ...
        anchor_forces.append(
            {
                "anchor_id": limb,
                "body_name": body_name,
                "force": [float(v) for v in wrench[0:3]],
                "torque": [float(v) for v in wrench[3:6]],
    ...
    result = {
        ...
        "image_debug": {
            ...
            "mediapipe_pixels": {
                name: [float(v) for v in image_landmarks_px[idx]]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--image", default=str(Path(__file__).parents[1] / "video" / "static.png"))
...
def main() -> None:
    ...
    print(
        json.dumps(
            {
                "joint_count": len(result["joint_torques"]),
                "anchor_count": len(result["anchor_forces"]),
