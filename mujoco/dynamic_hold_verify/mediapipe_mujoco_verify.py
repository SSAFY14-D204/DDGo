...
OPTIONAL_SHOULDER_SHRUG_JOINTS = ("shoulder_shrug_right", "shoulder_shrug_left")
...
LANDMARK_TO_STRESS_JOINTS = {
    LEFT_SHOULDER: ["shoulder_shrug_left", "shoulder1_left", "shoulder2_left"],
    RIGHT_SHOULDER: ["shoulder_shrug_right", "shoulder1_right", "shoulder2_right"],
...
def lerp_dict(prev_values: dict[str, float], next_values: dict[str, float], alpha: float) -> dict[str, float]:
    ...
    out: dict[str, float] = {}
    keys = tuple(dict.fromkeys(list(prev_values.keys()) + list(next_values.keys())))
    for key in keys:
...
def resolve_analysis_joints(model: mujoco.MjModel) -> list[str]:
    ...
    for name in MAJOR_JOINTS:
        if not joint_exists(model, name):
            continue
        joints.append(name)
...
def augment_joint_targets_for_model(model: mujoco.MjModel, joint_targets: dict[str, float]) -> dict[str, float]:
    augmented = dict(joint_targets)
    for joint_name in OPTIONAL_SHOULDER_SHRUG_JOINTS:
...
def extract_joint_angles(
    ...
    prev_points_local: dict[str, np.ndarray] | None = None,
) -> tuple[dict[str, float], dict[str, np.ndarray]]:
    raw_mp = np.array([[float(p.x), float(p.y), float(p.z)] for p in world_landmarks], dtype=np.float64)
    if calibration is None:
        return extract_joint_pose_targets(raw_mp, swap_lr=swap_lr)

    mapped_local = np.array([mp_to_mj(point) for point in raw_mp], dtype=np.float64)
    ...
    corrected_local = apply_inverse_depth_correction_to_mapped(
        mapped_local,
        segment_lengths_local,
        swap_lr=swap_lr,
    )
    corrected_local, _ = apply_two_link_pose_correction_to_mapped(
        corrected_local,
        segment_lengths_local,
        swap_lr=swap_lr,
        prev_points=prev_points_local,
    )
    return _extract_joint_pose_targets_from_mapped(corrected_local, swap_lr=swap_lr)


def build_forward_corrected_target_points(points: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    pivot_x = float(np.asarray(points["hip_mid"], dtype=np.float64)[0])
...
def swap_hand_target_points(points: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    swapped = {
    ...
    hand_pairs = [
        ...
        ("left_palm_contact", "right_palm_contact"),
    ]
    for left_key, right_key in hand_pairs:
        if left_key not in swapped or right_key not in swapped:
            continue
        left_value = np.asarray(swapped[left_key], dtype=np.float64).copy()
        right_value = np.asarray(swapped[right_key], dtype=np.float64).copy()
        swapped[left_key] = right_value
        swapped[right_key] = left_value
    return swapped
...
class DirectJointController:
    def __init__(
        ...
        mode: str,
        swap_lr: bool,
        freeze_root_in_kinematic: bool,
        swap_hand_targets_for_test: bool,
        ...
        self.freeze_root_in_kinematic = freeze_root_in_kinematic
        self.swap_hand_targets_for_test = swap_hand_targets_for_test
        self.calibration = calibration

        self.analysis_joints = resolve_analysis_joints(model)
        ...
        if self.mode == "dynamic" and len(self.actuator_ids) < len(self.analysis_joints):
            missing = sorted(list(set(self.analysis_joints) - set(self.actuator_ids.keys())))
        ...
        self.lhand_bid = body_id(model, "palm_contact_left")
        self.rhand_bid = body_id(model, "palm_contact_right")
        self.lelbow_bid = body_id(model, "lower_arm_left")
        self.relbow_bid = body_id(model, "lower_arm_right")
        self.lfoot_bid = body_id(model, "forefoot_contact_left")
        self.rfoot_bid = body_id(model, "forefoot_contact_right")
        self.lshoulder_bid = body_id(model, "upper_arm_left")
        self.rshoulder_bid = body_id(model, "upper_arm_right")
        self.debug_marker_mocap_ids: dict[str, int] = {}
        for marker_name in DEBUG_MARKER_TARGETS:
            bid = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, marker_name)
            if bid < 0:
                continue
            mocap_id = int(model.body_mocapid[bid])
            if mocap_id >= 0:
                self.debug_marker_mocap_ids[marker_name] = mocap_id
        ...
        self.base_shoulder_height = {
            "left": float(data.xpos[self.lshoulder_bid][2] - data.xpos[self.pelvis_bid][2]),
            "right": float(data.xpos[self.rshoulder_bid][2] - data.xpos[self.pelvis_bid][2]),
        }
        ...
        self.arm_ik_iters = 2
        self.elbow_ik_weight = 0.65
        self.prev_corrected_points_local: dict[str, np.ndarray] | None = None
    ...
    def _update_metric_calibration(self, points: dict[str, np.ndarray]) -> None:
        ...
        if self.offset_est is None:
            ...
            self.offset_est = 0.9 * self.offset_est + 0.1 * raw_offset

    def _build_target_points(self, points: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
        target_points = build_forward_corrected_target_points(points)
        if self.swap_hand_targets_for_test:
            target_points = swap_hand_target_points(target_points)
        return target_points

    def _estimate_shoulder_shrug_targets(self, points: dict[str, np.ndarray]) -> dict[str, float]:
        if self.scale_est is None:
            return {}
        axis_up = normalize(np.asarray(points["axis_up"], dtype=np.float64))
        hip_mid = np.asarray(points["hip_mid"], dtype=np.float64)
        targets: dict[str, float] = {}
        shoulder_specs = [
            ("left", "left_shoulder", "shoulder_shrug_left"),
            ("right", "right_shoulder", "shoulder_shrug_right"),
        ]
        for side, point_name, joint_name in shoulder_specs:
            if joint_name not in self.joint_limits:
                continue
            shoulder_point = np.asarray(points[point_name], dtype=np.float64)
            measured_height = float(np.dot(shoulder_point - hip_mid, axis_up)) * float(self.scale_est)
            target = measured_height - self.base_shoulder_height[side]
            lo, hi = self.joint_limits[joint_name]
            targets[joint_name] = float(np.clip(target, lo, hi))
        return targets
    ...
    def _refine_arm_targets_by_wrist_ik(
        ...
        try:
            target_points = self._build_target_points(points)
            ...
            arm_specs = [
                (
                    ...
                    "left_elbow",
                    self.lelbow_bid,
                    (["shoulder_shrug_left"] if "shoulder_shrug_left" in self.qpos_adr else [])
                    + ["shoulder1_left", "shoulder2_left"]
                    + (["shoulder3_left"] if "shoulder3_left" in self.qpos_adr else [])
                    + ["elbow_left"],
                ),
                (
                    "right_wrist",
                    ...
                    "right_elbow",
                    self.relbow_bid,
                    (["shoulder_shrug_right"] if "shoulder_shrug_right" in self.qpos_adr else [])
                    + ["shoulder1_right", "shoulder2_right"]
                    + (["shoulder3_right"] if "shoulder3_right" in self.qpos_adr else [])
                    + ["elbow_right"],
                ),
            ]

            for wrist_key, hand_bid, elbow_key, elbow_bid, joint_names in arm_specs:
                target_key = f"{wrist_key.replace('wrist', 'palm_contact')}"
                wrist_target_world = self._map_point(target_points.get(target_key, target_points[wrist_key]))
                elbow_target_world = self._map_point(target_points[elbow_key])

                for _ in range(self.arm_ik_iters):
                    mujoco.mj_forward(self.model, self.data)
                    hand_pos = self.data.xpos[hand_bid].copy()
                    elbow_pos = self.data.xpos[elbow_bid].copy()
                    hand_err = wrist_target_world - hand_pos
                    elbow_err = elbow_target_world - elbow_pos
                    if float(np.linalg.norm(hand_err)) < 0.015 and float(np.linalg.norm(elbow_err)) < 0.03:
                        break
                    ...
                    for jname in joint_names:
                        ...
                        mujoco.mj_forward(self.model, self.data)
                        hand_plus = self.data.xpos[hand_bid].copy()
                        elbow_plus = self.data.xpos[elbow_bid].copy()

                        self.data.qpos[adr] = self._clip_joint(jname, q0 - eps)
                        mujoco.mj_forward(self.model, self.data)
                        hand_minus = self.data.xpos[hand_bid].copy()
                        elbow_minus = self.data.xpos[elbow_bid].copy()

                        self.data.qpos[adr] = q0
                        hand_cols.append((hand_plus - hand_minus) / (2.0 * eps))
                        elbow_cols.append((elbow_plus - elbow_minus) / (2.0 * eps))

                    J_hand = np.column_stack(hand_cols)
                    J_elbow = np.column_stack(elbow_cols)
                    J = np.vstack([J_hand, self.elbow_ik_weight * J_elbow])
                    err = np.concatenate([hand_err, self.elbow_ik_weight * elbow_err])
    ...
    def prepare_targets(self, world_landmarks, timestamp_s: float) -> tuple[dict[str, float], dict[str, np.ndarray]]:
        raw_targets, points = extract_joint_angles(
            world_landmarks,
            swap_lr=self.swap_lr,
            calibration=self.calibration,
            prev_points_local=self.prev_corrected_points_local,
        )
        raw_targets = augment_joint_targets_for_model(self.model, raw_targets)
        self._update_metric_calibration(points)
        raw_targets.update(self._estimate_shoulder_shrug_targets(points))
        ...
        self.prev_corrected_points_local = {
            key: value.copy()
            for key, value in points.items()
    ...
    def apply_kinematic(self, targets: dict[str, float], points: dict[str, np.ndarray]) -> None:
        if self.freeze_root_in_kinematic:
            self.data.qpos[0:3] = self.root_pos0
            self.data.qpos[3:7] = self.root_quat0
        else:
    ...
    def compute_world_targets(self, points: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
        target_points = self._build_target_points(points)
        return {
            "pelvis": self._map_point(target_points["hip_mid"]),
            "left_wrist": self._map_point(target_points.get("left_palm_contact", target_points["left_wrist"])),
            "right_wrist": self._map_point(target_points.get("right_palm_contact", target_points["right_wrist"])),
            "left_ankle": self._map_point(target_points.get("left_forefoot_contact", target_points["left_ankle"])),
            "right_ankle": self._map_point(target_points.get("right_forefoot_contact", target_points["right_ankle"])),
        }

    def update_debug_markers(self, points: dict[str, np.ndarray]) -> None:
        if not self.debug_marker_mocap_ids or self.scale_est is None or self.offset_est is None:
            return
        target_points = self._build_target_points(points)
        for marker_name, point_name in DEBUG_MARKER_TARGETS.items():
            mocap_id = self.debug_marker_mocap_ids.get(marker_name)
            if mocap_id is None:
                continue
            point = target_points.get(point_name)
            if point is None:
                continue
            self.data.mocap_pos[mocap_id] = self._map_point(np.asarray(point, dtype=np.float64))
...
def run_self_check(
    ...
    with make_landmarker(task_path):
        _ = DirectJointController(
            ...
            mode=mode,
            swap_lr=swap_lr,
            freeze_root_in_kinematic=freeze_root_in_kinematic,
            swap_hand_targets_for_test=swap_hand_targets_for_test,
...
def run_live(
    ...
    existing_debug_markers = [str(name) for name in analysis_payload.get("debug_marker_names", [])]
    for marker_name in debug_marker_names:
        if marker_name not in existing_debug_markers:
            existing_debug_markers.append(marker_name)
    analysis_payload["debug_marker_names"] = existing_debug_markers
    ...
    print(f"[INFO] Hand target swap test={'ON' if swap_hand_targets_for_test else 'OFF'}")
            ...
            while viewer.is_running():
                ...
                if mode == "kinematic":
                    ...
                    if frame_targets is not None and frame_points is not None:
                        ...
                        controller.update_debug_markers(frame_points)
                    ...
                    if to_targets is not None and to_points is not None:
                        controller.update_debug_markers(to_points)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--xml", default=str(Path(__file__).with_name("humanoid_shoulder3_175.xml")), help="Path to MuJoCo XML")
    parser.add_argument(
        "--task-model",
    ...
    parser.add_argument(
        "--calibration-json",
        default=str(Path(__file__).with_name("calibration.json")),
        help="Calibration JSON used for inverse-depth correction and personalized segment priors",
    )
    ...
    parser.add_argument(
        "--freeze-root",
        action="store_true",
        help="In kinematic mode, keep torso/root fixed in the air to inspect limb mapping only",
    )
    parser.add_argument(
        "--swap-hand-targets-for-test",
        action="store_true",
        help="Swap only left/right hand targets for debugging whether the arms are aiming at opposite targets",
    )
...
def main() -> None:
    ...
    calibration_json_path = Path(args.calibration_json).resolve() if args.calibration_json else None
    ...
    swap_lr = bool(args.swap_lr) and not bool(args.no_swap_lr)
    analysis_payload = load_analysis_payload(analysis_config_path)
    calibration = (
        load_calibration_json(calibration_json_path)
        if calibration_json_path is not None and calibration_json_path.exists()
        else None
    )
    ...
    if args.self_check:
        run_self_check(
            ...
            mode=args.mode,
            swap_lr=swap_lr,
            freeze_root_in_kinematic=args.freeze_root,
            swap_hand_targets_for_test=args.swap_hand_targets_for_test,
