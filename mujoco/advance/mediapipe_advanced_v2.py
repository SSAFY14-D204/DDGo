...
def nlerp_quat(q0: np.ndarray, q1: np.ndarray, alpha: float) -> np.ndarray:
    a = float(np.clip(alpha, 0.0, 1.0))
    qa = normalize(q0.astype(np.float64))
    qb = normalize(q1.astype(np.float64))
    if float(np.dot(qa, qb)) < 0.0:
        qb = -qb
    return normalize((1.0 - a) * qa + a * qb)
...
class JointCalibrationTable:
    """JSON-backed calibration bridge between MediaPipe angles and MuJoCo joints.

    Each joint entry contains:
      - axis_direction: model hinge axis from humanoid.xml
      - sign_flip: multiplier applied to analytical angle
      - offset_rad: additive offset in radians
    """

    def __init__(self, path: Path, joints: dict[str, dict[str, float | list[float]]]) -> None:
        self.path = path
        self.joints = joints

    @staticmethod
    def _default_entry(axis: np.ndarray) -> dict[str, float | list[float]]:
        return {
            "axis_direction": [float(axis[0]), float(axis[1]), float(axis[2])],
            "sign_flip": 1.0,
            "offset_rad": 0.0,
        }

    @classmethod
    def load_or_create(cls, model: mujoco.MjModel, path: Path) -> "JointCalibrationTable":
        payload: dict[str, object] = {}
        if path.exists():
            payload = json.loads(path.read_text(encoding="utf-8-sig"))
        joints_payload = payload.get("joints") if isinstance(payload, dict) else None
        joints = joints_payload if isinstance(joints_payload, dict) else {}
        changed = False
        ...
        for name in MAJOR_JOINTS:
            ...
            if not isinstance(entry, dict):
                joints[name] = cls._default_entry(axis)
                changed = True
                continue

            if "axis_direction" not in entry or not isinstance(entry.get("axis_direction"), list):
                entry["axis_direction"] = [float(axis[0]), float(axis[1]), float(axis[2])]
                changed = True
            if "sign_flip" not in entry:
                entry["sign_flip"] = 1.0
                changed = True
            if "offset_rad" not in entry:
                entry["offset_rad"] = 0.0
                changed = True

        table = cls(path=path, joints={k: dict(v) for k, v in joints.items() if isinstance(v, dict)})
    ...
    def apply_value(self, joint_name: str, raw_value: float) -> float:
        entry = self.joints.get(joint_name, {})
        sign = float(entry.get("sign_flip", 1.0))
        offset = float(entry.get("offset_rad", 0.0))
        return sign * float(raw_value) + offset

    def apply_targets(self, raw_targets: dict[str, float]) -> dict[str, float]:
        return {name: self.apply_value(name, val) for name, val in raw_targets.items()}
...
class AdvancedRefinery:
    """Optional refinement layer for hand/foot end-effectors.

    Baseline analytical angles are applied first, then this class performs
    a few Jacobian IK iterations only for wrist/ankle endpoints.
    """
    ...
    def __init__(
        ...
        wall_axis: str,
        wall_z: float,
        wall_attraction_weight: float,
        wall_activation_dist: float,
    ) -> None:
        self.model = model
        self.data = data
        ...
        self.soft_z_snap = bool(soft_z_snap)
        self.wall_axis_index = {"x": 0, "y": 1, "z": 2}[wall_axis]
        self.wall_z = float(wall_z)
        self.wall_attraction_weight = max(float(wall_attraction_weight), 0.0)
        self.wall_activation_dist = max(float(wall_activation_dist), 0.0)

        self.refine_joint_names = list(HYBRID_REFINEMENT_JOINTS)
        self.all_qpos_adr = {jname: int(model.jnt_qposadr[joint_id(model, jname)]) for jname in MAJOR_JOINTS}
        self.refine_qpos_adr = []
        self.refine_dof_adr = []
        self.refine_joint_limits: dict[str, tuple[float, float]] = {}
        for jname in self.refine_joint_names:
            jid = joint_id(model, jname)
            self.refine_qpos_adr.append(int(model.jnt_qposadr[jid]))
            self.refine_dof_adr.append(int(model.jnt_dofadr[jid]))
            if model.jnt_limited[jid]:
                lo, hi = model.jnt_range[jid]
                self.refine_joint_limits[jname] = (float(lo), float(hi))
            else:
                self.refine_joint_limits[jname] = (-1e9, 1e9)

        self.effector_body = {
            "left_wrist": body_id(model, "hand_left"),
            "right_wrist": body_id(model, "hand_right"),
            "left_ankle": body_id(model, "foot_left"),
            "right_ankle": body_id(model, "foot_right"),
        }

    def _clip(self, joint_name: str, value: float) -> float:
        lo, hi = self.refine_joint_limits[joint_name]
        return float(np.clip(value, lo, hi))

    def _get_refine_q(self) -> np.ndarray:
        return np.array([float(self.data.qpos[adr]) for adr in self.refine_qpos_adr], dtype=np.float64)

    def _set_refine_q(self, q: np.ndarray) -> None:
        for i, jname in enumerate(self.refine_joint_names):
            self.data.qpos[self.refine_qpos_adr[i]] = self._clip(jname, float(q[i]))
    ...
    def refine(
        ...
        if not self.enabled:
            return base_targets, {"enabled": 0.0, "ik_effector_max_err_cm": 0.0}

        qpos_backup = self.data.qpos.copy()
        qvel_backup = self.data.qvel.copy()
        qacc_backup = self.data.qacc.copy()
        try:
            # Step 1: set baseline pose.
            for jname, q in base_targets.items():
                self.data.qpos[self.all_qpos_adr[jname]] = float(q)
            self.data.qvel[:] = 0.0
            self.data.qacc[:] = 0.0
            mujoco.mj_forward(self.model, self.data)

            q_ref = self._get_refine_q()
            target_pos = {k: map_point(points[k]) for k in self.effector_body}
            max_eff_err = 0.0

            # Step 2: small-step hybrid IK for wrist/ankle only.
            for _ in range(self.ik_iters):
                rows: list[np.ndarray] = []
                errs: list[np.ndarray] = []
                max_eff_err = 0.0

                for key, bid in self.effector_body.items():
                    jacp = np.zeros((3, self.model.nv), dtype=np.float64)
                    jacr = np.zeros((3, self.model.nv), dtype=np.float64)
                    mujoco.mj_jacBody(self.model, self.data, jacp, jacr, bid)
                    j = jacp[:, self.refine_dof_adr]
                    p = self.data.xpos[bid].copy()
                    e = target_pos[key] - p
                    max_eff_err = max(max_eff_err, float(np.linalg.norm(e)))
                    rows.append(j)
                    errs.append(e)

                    # Soft wall attraction as additional residual (no hard override).
                    if self.soft_z_snap:
                        dist = abs(float(target_pos[key][self.wall_axis_index]) - self.wall_z)
                        if dist <= self.wall_activation_dist:
                            jz = j[self.wall_axis_index : self.wall_axis_index + 1, :]
                            ez = np.array([self.wall_z - float(p[self.wall_axis_index])], dtype=np.float64)
                            rows.append(self.wall_attraction_weight * jz)
                            errs.append(self.wall_attraction_weight * ez)

                q_curr = self._get_refine_q()
                if self.regularization > 0.0:
                    w = math.sqrt(self.regularization)
                    rows.append(w * np.eye(len(self.refine_joint_names), dtype=np.float64))
                    errs.append(w * (q_ref - q_curr))

                j_stack = np.vstack(rows)
                e_stack = np.concatenate(errs)
                h = j_stack.T @ j_stack + (self.ik_damping * self.ik_damping) * np.eye(
                    len(self.refine_joint_names), dtype=np.float64
                )
                g = j_stack.T @ e_stack
                try:
                ...
                self._set_refine_q(q_next)
                mujoco.mj_forward(self.model, self.data)

            refined = dict(base_targets)
            for i, jname in enumerate(self.refine_joint_names):
                refined[jname] = float(self.data.qpos[self.refine_qpos_adr[i]])
            return refined, {"enabled": 1.0, "ik_effector_max_err_cm": float(max_eff_err * 100.0)}
        finally:
...
class DirectJointController:
    def __init__(
        ...
        mode: str,
        swap_lr: bool,
        calibration_table: JointCalibrationTable,
        advanced_refinery: AdvancedRefinery | None,
        ...
        self.advanced_refinery = advanced_refinery
    ...
    def prepare_targets(self, world_landmarks, timestamp_s: float) -> tuple[dict[str, float], dict[str, object]]:
        ...
        calibrated = self.calibration_table.apply_targets(raw_targets)
        clipped = {name: self._clip_joint(name, val) for name, val in calibrated.items()}
        ...
        refine_stats = {"enabled": 0.0, "ik_effector_max_err_cm": 0.0}
        if self.advanced_refinery is not None:
            targets, refine_stats = self.advanced_refinery.refine(targets, points, self._map_point)

        root_pos = self._map_point(points["hip_mid"]) + self.torso_from_pelvis
        root_quat = quat_from_axes(points["axis_forward"], points["axis_left"], points["axis_up"])

        frame_state: dict[str, object] = dict(points)
        frame_state["refine_stats"] = refine_stats
        frame_state["root_pos"] = root_pos
        frame_state["root_quat"] = root_quat
        return targets, frame_state

    def apply_kinematic(self, targets: dict[str, float], frame_state: dict[str, object]) -> None:
        self.set_root_pose(
            np.asarray(frame_state["root_pos"], dtype=np.float64),
            np.asarray(frame_state["root_quat"], dtype=np.float64),
        )
    ...
    def set_root_pose(self, root_pos: np.ndarray, root_quat: np.ndarray) -> None:
        self.root_target_cache = root_pos.copy()
        self.root_quat_cache = normalize(root_quat.copy())
    ...
    def compute_metrics(self, targets: dict[str, float], frame_state: dict[str, object]) -> dict[str, object]:
        ...
        return {
            ...
            "refine_stats": frame_state.get("refine_stats", {"enabled": 0.0, "ik_effector_max_err_cm": 0.0}),
        }
...
def run_diagnostic_joint_sweep(
    xml_path: Path,
    ...
    calibration = JointCalibrationTable.load_or_create(model, calibration_path)
    if damping_scale > 0 and abs(damping_scale - 1.0) > 1e-6:
    ...
    print(f"[INFO] Diagnostic sweep start: {len(MAJOR_JOINTS)} joints, step={sweep_deg:.2f}deg")
    with mujoco.viewer.launch_passive(model, data) as viewer:
        for jname in MAJOR_JOINTS:
            ...
            if model.jnt_limited[jid]:
                lo, hi = model.jnt_range[jid]
            axis = calibration.joints.get(jname, {}).get("axis_direction", [0.0, 0.0, 0.0])
            sign_flip = float(calibration.joints.get(jname, {}).get("sign_flip", 1.0))
            print(f"[DIAG] {jname}: axis={axis}, sign_flip={sign_flip:+.1f}")

            for direction in (1.0, -1.0):
                data.qpos[:] = qpos0
                target = float(np.clip(data.qpos[adr] + direction * sweep_rad, lo, hi))
        ...
        mujoco.mj_forward(model, data)
        viewer.sync()
    print("[OK] Diagnostic sweep finished")
...
def run_self_check(
    ...
    model, data = build_model(xml_path)
    calibration_table = JointCalibrationTable.load_or_create(model, calibration_path)
    advanced_refinery = AdvancedRefinery(
        model=model,
        data=data,
        enabled=(refinement_mode == "hybrid"),
        ik_iters=refine_iters,
        ik_damping=refine_damping,
        ik_step_scale=refine_step_scale,
        regularization=refine_regularization,
        soft_z_snap=soft_z_snap,
        wall_axis=wall_axis,
        wall_z=wall_z,
        wall_attraction_weight=wall_attraction_weight,
        wall_activation_dist=wall_activation_dist,
    )
    with make_landmarker(task_path):
        _ = DirectJointController(
            ...
            advanced_refinery=advanced_refinery,
...
def run_live(
    ...
    print(f"[INFO] Calibration table={calibration_path}")
    print(
        f"[INFO] Refinement mode={refinement_mode} "
        f"(iters={refine_iters}, damping={refine_damping:.4f}, step={refine_step_scale:.2f}, reg={refine_regularization:.4f})"
    )
    print(
        f"[INFO] Soft Z-snap={'ON' if soft_z_snap else 'OFF'} "
        f"(axis={wall_axis}, wall={wall_z:.3f}, weight={wall_attraction_weight:.3f}, dist={wall_activation_dist:.3f})"
    )
            ...
            while viewer.is_running():
                    ...
                    while data.time + model.opt.timestep * 0.5 < target_sim_time:
                        ...
                        if to_state is not None:
                            if from_state is None:
                                root_pos = np.asarray(to_state["root_pos"], dtype=np.float64)
                                root_quat = np.asarray(to_state["root_quat"], dtype=np.float64)
                            else:
                                p0 = np.asarray(from_state["root_pos"], dtype=np.float64)
                                p1 = np.asarray(to_state["root_pos"], dtype=np.float64)
                                q0 = np.asarray(from_state["root_quat"], dtype=np.float64)
                                q1 = np.asarray(to_state["root_quat"], dtype=np.float64)
                                root_pos = (1.0 - phase) * p0 + phase * p1
                                root_quat = nlerp_quat(q0, q1, phase)
                            controller.set_root_pose(root_pos, root_quat)
                ...
                if metrics is not None:
                    ...
                    record = {
                        ...
                        "refine_stats": metrics.get("refine_stats", {}),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Calibration-first bridge: MediaPipe analytical mapping + optional hybrid refinement",
    )
    ...
    parser.add_argument(
        "--calibration-json",
        default=str(Path(__file__).with_name("artifacts") / "joint_axis_calibration.json"),
        help="Calibration JSON for joint sign/offset/axis bridge",
    )
    ...
    parser.add_argument(
        ...
        choices=["off", "hybrid"],
        default="off",
        help="Hybrid refinement mode: baseline-only or analytical+small-step Jacobian",
    )
    parser.add_argument("--refine-iters", type=int, default=4, help="Hybrid refinement IK iterations (3-5 recommended)")
    parser.add_argument("--refine-damping", type=float, default=0.01, help="Hybrid IK damping")
    parser.add_argument("--refine-step-scale", type=float, default=0.5, help="Hybrid IK step scale")
    parser.add_argument("--refine-regularization", type=float, default=0.02, help="Hybrid IK regularization to baseline")
    parser.add_argument("--soft-z-snap", action="store_true", help="Enable soft wall-plane attraction in refinement")
    parser.add_argument("--wall-axis", choices=["x", "y", "z"], default="x", help="Wall plane axis")
    parser.add_argument("--wall-z", type=float, default=0.9, help="Wall plane position on selected axis")
    parser.add_argument(
        "--wall-attraction-weight",
        type=float,
        default=0.35,
        help="Soft Z-snap residual weight in refinement IK",
    )
    parser.add_argument(
        "--wall-activation-dist",
        type=float,
        default=0.12,
        help="Soft Z-snap activation distance to wall plane (m)",
    )
    parser.add_argument(
        "--diagnostic-joint-sweep",
        action="store_true",
        help="Rotate each joint by +/- sweep degrees for visual axis calibration",
    )
    parser.add_argument("--diagnostic-sweep-deg", type=float, default=10.0, help="Diagnostic sweep angle per joint")
    parser.add_argument("--diagnostic-hold-sec", type=float, default=0.35, help="Hold time per direction in diagnostic")
    parser.add_argument(
        "--error-log",
        default=str(Path(__file__).with_name("artifacts") / "mapping_error_advanced_v2.jsonl"),
    )
...
def main() -> None:
    ...
    if args.diagnostic_joint_sweep:
        run_diagnostic_joint_sweep(
            xml_path=xml_path,
            calibration_path=calibration_path,
            sweep_deg=args.diagnostic_sweep_deg,
    ...
    if args.self_check:
        run_self_check(
            ...
            soft_z_snap=args.soft_z_snap,
            wall_axis=args.wall_axis,
            wall_z=args.wall_z,
            wall_attraction_weight=args.wall_attraction_weight,
            wall_activation_dist=args.wall_activation_dist,
