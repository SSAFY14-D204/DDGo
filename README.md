...
```
[S14P21D204-2][BE]fix: JWT 토큰 만료 시간 수정
```
...
### 예시
```
[S14P21D204-2][BE]Feat 회원가입 및 로그인 API 구현
[S14P21D204-3][BE]Fix 게임 점수 계산 오류 수정
[S14P21D204-4][BE]Refactor 공통 응답 DTO 구조 개선
```
...
| **Story** | 사용자 관점의 기능 단위 | 사용자가 로그인할 수 있다 |
...
```
Epic: [회원] 회원 관리
  ├── Story: 사용자가 소셜 로그인을 할 수 있다
  │     ├── task: [BE] OAuth2 카카오 로그인 API 구현
  │     ├── task: [BE] JWT 토큰 발급 로직 구현
  │     └── task: [FE] 소셜 로그인 버튼 UI 구현
  ├── Story: 사용자가 프로필을 수정할 수 있다
  │     ├── task: [BE] 프로필 수정 API 구현
  │     └── task: [FE] 프로필 수정 페이지 구현
  └── Task: [DB] 회원 테이블 스키마 설계
```
...
```
[사용자 유형]이/가 [행동]을/를 할 수 있다
```

### 예시

```
사용자가 이메일로 회원가입을 할 수 있다
관리자가 사용자 목록을 조회할 수 있다
게스트가 게임 방 목록을 확인할 수 있다
```
...
```
S14P21D204 Sprint 1 (02/23 ~ 03/01)
S14P21D204 Sprint 2 (03/02 ~ 03/08)
```
