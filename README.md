...
이 저장소는 오픈소스가 아닙니다. 별도의 서면 허가 없이 DDGo의 원본
소스 코드, 문서, UI/UX 디자인, 디자인 시스템 및 시각 자산, 모델과 기타
자료를 사용·복제·수정·재배포·상업적으로 이용하거나 2차적 저작물을
만드는 것, AI·머신러닝 시스템의 학습·파인튜닝·평가·검색 데이터로
활용하는 것을 허가하지 않습니다. 제3자 구성요소에는 각 원래 라이선스와
이용 조건이 적용됩니다.

단, 별도의 서면 허가가 있거나 관련 법령 및 GitHub 이용약관이 허용하는
범위는 제외됩니다. 이 안내는 요약이며, 내용이 충돌하는 경우 루트의
[LICENSE](./LICENSE) 영문 원문이 우선합니다.
