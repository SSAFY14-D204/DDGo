...
class CalendarUiStateMapperTest {
    ...
    @Test
    fun `createCalendarUiState builds sunday first grid`() {
        ...
        assertEquals(LocalDate.of(2026, 3, 7), state.weeks.first().last().date)
    }

    @Test
    fun `color filter builds distinct color markers`() {
        ...
        val state = CalendarUiStateMapper.createCalendarUiState(
            ...
            entries = listOf(
                entry(1L, targetDate, "더클라임 강남", "보라"),
                entry(2L, targetDate, "더클라임 강남", "분홍"),
                entry(3L, targetDate, "서울숲 클라이밍", "노랑"),
                entry(4L, targetDate, "서울숲 클라이밍", "보라")
            ),
        ...
        assertEquals(listOf("보라", "분홍", "노랑"), day.colorMarkers.map { it.label })
    }
