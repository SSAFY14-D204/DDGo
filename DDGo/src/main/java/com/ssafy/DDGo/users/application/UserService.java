...
import com.ssafy.DDGo.users.domain.AutoGeneratedNicknamePolicy;
...
public class UserService {
    ...
    private static final int AUTO_NICKNAME_RANDOM_RETRIES = 50;
    private static final AutoGeneratedNicknamePolicy AUTO_GENERATED_NICKNAME_POLICY =
            new AutoGeneratedNicknamePolicy();
    ...
    public void registerUser(UserRegisterRequest request) {
        ...
        PasswordPolicy.validatePasswordRules(normalizedEmail, null, request.getPassword());

        User user = User.builder()
                .username(normalizedEmail)
                .email(normalizedEmail)
                .password(passwordEncoder.encode(request.getPassword()))
                .nickname(generateAvailableAutoNickname(request.getPassword()))
                .build();
    ...
    private UserLoginResponse loginOrRegisterSocialUser(SocialUserProfile socialUser) {
        ...
        User user = userRepository.save(User.builder()
                ...
                .password(null)
                .nickname(generateAvailableAutoNickname(null))
                .build());
    ...
    private String generateAvailableAutoNickname(String password) {
        for (int retry = 0; retry < AUTO_NICKNAME_RANDOM_RETRIES; retry++) {
            String candidate = AUTO_GENERATED_NICKNAME_POLICY.randomCandidate(random);
            if (isAvailableAutoNickname(candidate, password)) {
                return candidate;
            }
        }

        int totalCandidates = AUTO_GENERATED_NICKNAME_POLICY.size();
        int startIndex = random.nextInt(totalCandidates);
        for (int offset = 0; offset < totalCandidates; offset++) {
            String candidate = AUTO_GENERATED_NICKNAME_POLICY.candidateAt((startIndex + offset) % totalCandidates);
            if (isAvailableAutoNickname(candidate, password)) {
                return candidate;
            }
        }

        throw new CustomException(ErrorCode.INTERNAL_SERVER_ERROR, "자동 닉네임 생성에 실패했습니다.");
    }

    private boolean isAvailableAutoNickname(String nickname, String password) {
        if (userRepository.countByNicknameIncludingDeleted(nickname) > 0) {
            return false;
        }
        ...
        return !password.toLowerCase(Locale.ROOT).contains(nickname.toLowerCase(Locale.ROOT));
    }
